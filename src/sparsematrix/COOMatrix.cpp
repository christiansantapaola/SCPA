...
COOMatrix::COOMatrix(const float *Matrix, int rows, int cols) {
    row_size = rows;
    col_size = cols;
...
int COOMatrix::getNumNonZeroElements() const {
    return num_non_zero_elements;
}

int COOMatrix::getRowSize() const {
    return row_size;
}

int COOMatrix::getColSize() const {
    return col_size;
}
